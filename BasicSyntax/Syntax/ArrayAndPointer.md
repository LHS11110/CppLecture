...
하지만 2차원 배열을 이중 포인터로 가르키려 하면 에러가 발생하는데, 원인은 `이중 포인터로는 인덱싱을 할 수 없기 때문`이다.
2차원 배열의 인덱싱 연산중 `1차원 배열의 원소 개수`인 `arr1_size`를 알 수 없어 인덱싱이 불가능하다.
하지만 `이를 알 수 있게 해주는 것이 배열 포인터`이다. 위처럼 ```[2]```를 명시하면 `1차원 배열의 원소 개수가 2개`인 `2차원 배열을 가르키는 포인터`를 의미하게 된다.
...
코드를 해석하면 `2차원 배열의 원소 개수는 3개`, `1차원 배열의 원소 개수도 3개`인 `3차원 배열을 가르키는 포인터`를 의미한다.
