...
자료형은 이러한 의미를 가지기에 주소에 접근하여 데이터를 연산을 하려면 명시된 자료형이 필요하다.
...
반대로 단순히 주소만을 나타내면 접근조차 못하는데 무슨 의미가 있겠나 싶지만 이것을 통해 다양한 것을 구현할 수 있다. 또한 포인터는 자료형의 주소를 가르키는 변수이다. 함수 또한 파생 자료형에 속하는데, 때문에 함수를 가르키는 포인터가 존재한다.
