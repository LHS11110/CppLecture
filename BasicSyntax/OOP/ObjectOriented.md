...
## 객체 생성 과정
- 생성하기전 컴파일러는 기본적으로 객체의 정적 멤버 변수와 메소드를 메모리에 할당한다.
1. 객체의 필요한 공간을 할당한다.
2. 생성자를 호출한다.
3. 멤버 변수는 멤버 이니셜라이저를 통해 초기화한다.
4. 생성자의 코드블록이 실행된다.
## 소멸자
객체는 생성자를 통해 인스턴스화되면 필요없는 시점에 이를 다시 반환하는 소멸자를 호출해야 한다. 소멸자는 대부분의 상황에서는 다룰 일이 없지만 동적 할당이라는 개념에서는 함께 사용되야 한다.
